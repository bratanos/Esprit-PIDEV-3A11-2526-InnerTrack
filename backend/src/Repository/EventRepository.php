...
class EventRepository extends ServiceEntityRepository
{
    ...
    public function filterEvents(?string $q, ?int $type, ?string $period, ?string $avail): array
    {
        ...
        // Notes on "avail" filter:
        // 'available' or 'full'. It involves joining Inscription which is more complex in DQL because we need to count them.
        // For performance and simplicity, we can fetch all filtered by the other criteria,
        // and let the controller do the final filter for capacity, or write a subquery.
        // Let's use a subquery for 'avail' filter.
        if ($avail === 'available') {
            $qb->andWhere(
                'e.capacite > (
